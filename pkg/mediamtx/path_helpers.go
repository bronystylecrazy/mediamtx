...
// PathHelper provides utility methods for path operations
...
// ValidatePathName validates a path name according to MediaMTX rules
...
func (a *PathAnalyzer) AnalyzePathSource(path *conf.Path) map[string]interface{} {
	...
	case regexp.MustCompile(`^[a-zA-Z0-9_\-]+$`).MatchString(sourceStr):
		...
		analysis["description"] = "Redirect to another path"
...
// AnalyzePathStatus provides detailed status information about an active path
...
// BuildPathURL builds a complete URL for a path based on the protocol
